        ...
        <section class="w-fit mx-auto grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 justify-items-center justify-center gap-y-20 gap-x-14 mt-10 mb-5">
            ...
            v-for="product in items" :key="product.id">
                <RouterLink :to="`/detail/${product.id}`">
                    <img :src="product.thumbnail" alt="Product" class="h-80 w-72 object-cover rounded-t-xl" />
                    <div class="px-4 py-3 w-72">
                        ...
                        <div class="flex items-center">
                            <p class="text-lg font-semibold text-black cursor-auto my-3">${{ product.discountPercentage }}</p>
                            <del>
...
export default {
    ...
    created() {
       // this.$store.dispatch('cart/fetchProducts')
        this.$store.dispatch('product/getAllProducts',{page:1})
        this.increment()
    },
    computed: {
       /*  products() {
